...
const WalletTable = () => {
  ...
  const copyWalletList = walletList.map((item) => ({
    ...
    profit: (
      (getLatestPrice(item.curr) * item.amount).toFixed(2) -
      item.amount * item.price
    ).toFixed(2),
  }));

  const sumProfit = copyWalletList
    .reduce((sum, { profit }) => sum + Number(profit), 0)
    .toFixed(2);
        ...
        <table>
          ...
          <tfoot>
            <tr>
              ...
              <td className="sum-profit" colSpan="2">
                {sumProfit} PLN
              </td>
              <td></td>
            </tr>
