import {
  getLatesPriceOfCurrAPI,
  getPrevPriceOfCurrAPI,
  exchangeCurrAPI,
} from './exchange.api';
...
export const getPrevPriceOfCurrAction =
  (curr, date) => (dispatch, getState) => {
    return getPrevPriceOfCurrAPI(curr, date)
      .then((resp) => dispatch(loadPrevPriceOfCurrAction(resp)))
      .catch((err) => dispatch(setErrorAction(err)));
  };

export const getExchangeCurrAction =
  (fromCurr, toCurr, amount) => (dispatch, getState) => {
